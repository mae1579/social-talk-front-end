...
export function Sidebar() {
    ...
    function joinRoom(room, isPublic = true) {
        if (!user) {
            return alert("Proszę zaloguj się przed skorzystaniem z chatu !");
        }
    ...
    return (
        <>
            <h2>Dostępne pokoje rozmów:</h2>
            <ListGroup>
                {rooms.map((room, idx) => (
                    <ListGroup.Item key={idx} onClick={() => joinRoom(room)} active={room == currentRoom}
            ...
            {members.map((member) => (
                <ListGroup.Item key={member.id} style={{cursor: "pointer"}}
                    ...
                    <Row>
                        ...
                        <Col xs={1}>
                            <span
                                className="badge rounded-pill bg-primary">{user.newMessages[orderIds(member._id, user._id)]}</span>
                        </Col>
                    </Row>
                </ListGroup.Item>
            ))}
